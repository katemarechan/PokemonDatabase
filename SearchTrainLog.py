...
def fetch_users(username, password, filters=None):
    try:
...
def display_users(text_field=None, username=None, password=None, filters=None):
    results = fetch_users(username, password, filters)  # Przekaż filtry do funkcji pobierającej dane
...
class TrainerGUI(Frame):
    ...
    def create_text_elements(self):
        ...
        display_users(text_field=self.text_field, username=self.username, password=self.password)
